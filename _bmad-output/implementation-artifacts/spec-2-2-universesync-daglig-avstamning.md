...
> **SUPERSEDED 2026-09-10.** Depends on `BorsdataAdapter` (dropped — Börsdata's API requires
> paid Pro). To be regenerated by `bmad-build` after Story 2.1 v2 lands, against
> `AvanzaUniverseAdapter`. The reconciliation design (diff / delisting / reactivation /
> timeboxed id-resolution / `universe_sync` churn row) carries forward; Avanza `orderbookId`
> now comes free from the listing, so only the Nordnet id is looked up. See
