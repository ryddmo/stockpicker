...
Independent code review (2026-09-10) — bmad-code-review, 4 layers (blind-hunter, edge-case-hunter, verification-gap, acceptance-auditor). 1 patch, 2 deferred, 12 rejected.

- [x] [Review][Patch] Live-verification observability is incomplete and untested [bin/show-universe.php; src/Adapter/BorsdataAdapter.php buildUniverse(); tests/Adapter/BorsdataAdapterTest.php] — `bin/show-universe.php` stdout (what the operator is told to paste back) showed only per-list counts + a 10-row sample; the `/v1/markets` id→name→label map and the instrument-type ids seen on target markets reached only the `info` log, and no dropped-row tally was emitted anywhere. No test asserted the `borsdata: universe built` info record. **Applied 2026-09-10:** (a) `bin/show-universe.php` attaches a `Monolog\Handler\TestHandler`, captures the `borsdata: universe built` record and prints the marketId→label map, kept/seen instrument types, and dropped-row tally to stdout with a pin-your-constants hint; (b) `buildUniverse()` now tracks `$dropped` (`non_object` / `bad_isin` / `empty_name`) and adds it to the info context (non-object handling moved out of `instruments()` into the one loop); (c) new test `testLogsTheUniverseBuiltInfoRecordWithTheMarketMapTypesCountsAndDrops`. Full suite green (171 tests).

- [x] [Review][Defer] No listing-status / tradability filter — delisted or halted instruments may enter the universe [src/Adapter/BorsdataAdapter.php buildUniverse()] — deferred: maybe-false. `buildUniverse()` filters only by target `marketId` + `instrument` type 0/1, never `listingDate` or a delisted flag. Whether `/v1/instruments` on the four target marketIds returns only currently-listed instruments cannot be determined without the live API. Settle on the mandatory live run: spot-check a known-delisted name; if present, add a listing-status filter. Story 2.2's reconcile partly compensates.

- [x] [Review][Defer] docs/deploy.md config checklist does not mention the new borsdata.api_key [docs/deploy.md] — deferred: not load-bearing until Story 2.2 wires `UniverseSync` into `/cron/refill`; `config.php.dist` already carries a commented section. Add `borsdata.api_key` to the deploy runbook's config step with Story 2.2.

### Rejected

- **false** — API key leaks into logs: `requestJson` builds every thrown/logged message from the bare `$uri` (`BASE_URI` + path, no query string); the authKey travels in Guzzle's `query` option; the underlying Guzzle exception that embeds the full URL is chained as `$previous` and never logged; connection-error curl messages are host/port-level.
- **false** — multiple market ids → one label causes double-counting: each instrument carries a single `marketId` and is emitted once; folding "First North" segments into one label is intended and matches `Store\Instrument::$list`; the `array_unique` completeness check still requires all four labels.
- **false** — `bin/show-universe.php` bypasses a shared bootstrap HTTP client: `bootstrap.php` returns only config + logger; every entry point (`bin/resolve-ids.php`, `public_html/index.php`) constructs its own `Client`; this script follows that exact pattern.
- **false** — `countryId` consumed without schema validation: pass-1 patch #1's "all four target labels must correlate" check raises `SchemaMismatch` for any `countryId` drift that affects a target market; a non-target entry missing `countryId` is correctly skipped.
- **false** — `bin/show-universe.php` Guzzle `Client` outside `src/Adapter/` violates AC-2: the AC targets source-specific HTTP logic (URLs/parsing/JSON shape); composition-root client construction follows the `bin/resolve-ids.php` precedent (also pass-1 triage #19).
- **low** — `Config::borsdataApiKey()` accepts the `'change-me'` placeholder: failure is a loud `401 → SchemaMismatch` on first call / the mandatory live run; matches existing `cronToken()` behavior, already a tracked Story-1.1 deferred item that this pairs with.
- **low** — `UniverseEntry` does no field validation: matches the `NormalizedRow` pattern (VO is a plain carrier, adapter owns validation); the only constructor call site validates all three fields.
- **low** — `withOneRetry` wraps both HTTP calls / no test for a transient on `/v1/instruments`: `/v1/markets` re-fetch is idempotent; ≤4 calls vs a 100/10 s ceiling; matches `NordnetAdapter::fetch` (pass-1 triage #12).
- **low** — `status: 'done'` while a Story-2.2-blocking item is open: the fix is spec-file metadata; the sprint tracker correctly shows `review`; the deferred item is logged with operator sign-off (2026-09-10).
- **rejected (fix edits the spec under review)** — frozen Intent says "a real Börsdata key is available now" but none was on the implementing machine: reconciling the frozen block is the human's call; the divergence is on record (pass-1 triage #7 + operator decision).
- **rejected (fix edits the spec under review)** — Design Notes code sketch still shows the loose `/^[A-Z0-9]{12}$/` regex and no empty-name guard: stale vs shipped code; spec hygiene, not a code defect.
- **already tracked** — guessed constants (`EQUITY_TYPE_IDS`, market-name strings) encoded in synthetic fixtures: fully covered by the existing `deferred-work.md` entry as a Story-2.2 blocker with exact remediation.
